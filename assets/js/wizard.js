document.addEventListener("DOMContentLoaded", function () {
    ...
    function getValidOptions(type, subtype, currentHeight, currentColor) {
        ...
        if (typeof HHDC !== 'undefined' && HHDC.config && HHDC.config.mappings) {
            Object.values(HHDC.config.mappings).forEach((map) => {
              ...
              if (colorMatch && map.thick_mm && map.thick_mm > 0) validHeights.add(map.thick_mm);
        
              let heightMatch = true;
              if (currentHeight && map.thick_mm && map.thick_mm != currentHeight) heightMatch = false;
    ...
    form.addEventListener("submit", async function (e) {
        ...
        try {
            const resp = await fetch(HHDC.rest.base + "/calc", {
                method: "POST", 
                headers: { "Content-Type": "application/json", "X-WP-Nonce": HHDC.nonce },
